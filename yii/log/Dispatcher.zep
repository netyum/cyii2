...
class Dispatcher extends Component
{
    ...
    public function init()
    {
        parent::init();
        var name, target;
        for name, target in this->targets {
            if ! (target instanceof Target) {
                let this->targets[name] = BaseYii::createObject(target);
            }
        }
    }
    ...
    public function getLogger()
    {
        if (this->_logger === null) {
            this->setLogger(BaseYii::getLogger());
        }
        return this->_logger;
    }
    ...
    {
        let this->_logger = value,
            value->dispatcher = this;
    }
